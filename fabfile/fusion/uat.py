...
def build():
    with settings(use_shell=False, always_use_pty=False):
        ...
        with cd('/srv/build/fusion/uat'):
            ...
            run('docker build --tag=fusionapp/fusion:uat --file=docker/fusion.docker .')
...
def deploy():
    with settings(use_shell=False, always_use_pty=False):
        ...
        run('docker run --rm --volume=/srv/db/fusion:/db --volume=/srv/certs:/srv/certs fusionapp/fusion:uat upgrade')
