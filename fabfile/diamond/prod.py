...
def deploy():
    with settings(use_shell=False, always_use_pty=False):
        ...
        run('docker run --rm --env DIAMOND_PROD=1 --volume=/srv/db/diamond:/db --volume=/srv/certs:/srv/certs fusionapp/diamond:prod upgrade')
        run('docker run --detach --restart=always --name=diamond --env DIAMOND_PROD=1 --volume=/srv/db/diamond:/db --volume=/srv/certs:/srv/certs --publish=41.72.130.254:443:443 --publish=41.72.130.254:8021:8021 --workdir=/db fusionapp/diamond:prod')
